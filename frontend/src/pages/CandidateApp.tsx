...
export function CandidateApp({ onBack }: Props) {
  ...
  const handleCpfSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data.status === 'DOCS_SENT' || res.data.status === 'APPROVED') {
        setStep('done');
      } else {
        setStep('form');
      }
  ...
  // CEP: busca autom\u00e1tica com debounce de 600ms
  ...
  const handleMultiFile = async (questionId: string, newFiles: FileList) => {
    ...
    const combined = [...current, ...Array.from(newFiles)].slice(0, 3);
  ...
  const handleNext = async () => {
    setLoading(true); setError('');
    const phase = process?.phases?.[currentPhaseIndex];
    // Salva endere\u00e7o se a fase tiver CEP
  ...
  if (step === 'form' && process) {
    ...
    return (
      <div className="min-h-screen bg-slate-50 py-8 px-4">
        ...
        <div className={`fixed top-4 right-4 z-50 transition-all duration-300 ${savedBanner ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'}`}>
        ...
        <div className="max-w-lg mx-auto">
          ...
          {currentPhaseIndex > 0 && (
            <div className="bg-teal-50 border border-teal-200 rounded-xl p-3 mb-4 flex items-center gap-2">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#0d9488" strokeWidth="2"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z" /><path d="M12 8v4l3 3" /></svg>
                ...
                return (
                  <div key={q.id} className={`space-y-1.5 ${isCorrection ? 'ring-2 ring-amber-400 rounded-lg p-3 -mx-3' : ''}`}>
                    ...
                    {q.type === 'CEP' && (
                      <div className="relative">
                        <input type="text" value={answers[q.id] || ''} maxLength={9}
                          ...
                          className="w-full px-3.5 py-2.5 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 outline-none pr-10"
                        ...
                        {!cepLoading && answers[q.id]?.replace(/\D/g, '').length === 8 && !cepError && (
                          <div className="absolute right-3 top-1/2 -translate-y-1/2">
                      ...
                      <div className="border-2 border-dashed border-slate-300 rounded-lg p-4 text-center hover:border-teal-400 transition-colors">
                        {answers[q.id] === 'ARQUIVO' ? (
                          ...
                          <label className="cursor-pointer block">
                            ...
                            <p className="text-slate-400 text-xs mt-1">PDF ou imagem</p>
                            <input type="file" className="hidden" accept="image/*,.pdf"
                              onChange={e => e.target.files?.[0] && handleFile(q.id, e.target.files[0])} />
                          </label>
                        )}
                    ...
                    {q.type === 'MULTI_FILE' && (() => {
                      ...
                      const displayFiles = localFiles.length > 0 ? localFiles.map(f => f.name) : serverDocs.map(d => d.fileName);
                      const canAdd = displayFiles.length < 3;
                      return (
                        <div className="space-y-2">
                          {displayFiles.map((name, i) => (
                            <div key={i} className="flex items-center gap-2 bg-teal-50 border border-teal-200 rounded-lg px-3 py-2">
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#0d9488" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>
                              <span className="text-xs text-teal-700 flex-1 truncate">{name}</span>
                              {localFiles.length > 0 && (
                          ...
                          {canAdd && (
                            <label className="flex items-center gap-2 border-2 border-dashed border-slate-300 rounded-lg px-3 py-2 cursor-pointer hover:border-teal-400 transition-colors">
                              ...
                              <span className="text-xs text-slate-500">
                                {displayFiles.length === 0 ? 'Adicionar arquivo (m\u00e1x 3)' : `Adicionar mais (${3 - displayFiles.length} restante${3 - displayFiles.length > 1 ? 's' : ''})`}
                              </span>
                          ...
                          {displayFiles.length === 3 && (
                            <p className="text-xs text-slate-400">M\u00e1ximo de 3 arquivos atingido.</p>
                          )}
                        </div>
                      );
                    })()}
